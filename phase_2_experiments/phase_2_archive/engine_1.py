...
# The Creative Prisim (Prisim)
# Version 1.2
#
# Prisim Experimental Instrument — Do Not Refactor Without Explicit Approval
#
# Changes in v1.2:
...
def validate_stage(blackboard: dict, stage_name: str) -> bool:
    """
    Validate that a pipeline stage completed successfully.
    Pure Python — no API call. Halts with a descriptive error if validation fails.

    Called after each stage gate in the notebook. Catches silent failures
    (empty fields, missing cycles, unpopulated candidate sets) before they
    propagate silently to downstream cells.

    Args:
        blackboard : the current session blackboard
        stage_name : one of "brief" | "ideation" | "critic" | "candidate_set"
    ...
    if stage_name == "brief":
        ...
        if missing:
            raise RuntimeError(
                f"VALIDATION FAILED — brief stage: missing required fields: {missing}\n"
                "The Director did not produce a complete brief.\n"
                "Check the brief parsing cell and model output above."
            )
        ...
        if not cycles:
            raise RuntimeError(
                "VALIDATION FAILED — ideation stage: no ideation cycles found.\n"
                "The Creator did not produce any directions.\n"
                "Check the Creator call and model output above."
            )
        proposals = cycles[-1].get("creator_proposals", [])
        if not proposals or not proposals[0].get("raw_response", "").strip():
            raise RuntimeError(
                "VALIDATION FAILED — ideation stage: creator_proposals is empty.\n"
                "The Creator call may have failed or returned no content.\n"
                "Check the Creator call and model output above."
            )
        ...
        if not cycles:
            raise RuntimeError(
                "VALIDATION FAILED — critic stage: no ideation cycles found.\n"
                "Run ideation before critic validation."
            )
        feedback = cycles[-1].get("critic_feedback", [])
        if not feedback or not feedback[0].get("raw_response", "").strip():
            raise RuntimeError(
                "VALIDATION FAILED — critic stage: critic_feedback is empty.\n"
                "The Critic call may have failed or returned no content.\n"
                "Check the Critic call and model output above."
            )

    elif stage_name == "candidate_set":
        candidates = blackboard.get("candidate_set", [])
        if not candidates:
            raise RuntimeError(
                "VALIDATION FAILED — candidate_set stage: candidate_set is empty.\n"
                "The Director review did not produce any candidates.\n"
                "Check the Director review JSON parsing output above."
            )

    else:
        raise ValueError(
            f"Unknown stage_name: '{stage_name}'.\n"
            f"Valid options: brief | ideation | critic | candidate_set"
        )
...
## Creative Prisim — The Creative Prisim
**Session:** {session_id}
