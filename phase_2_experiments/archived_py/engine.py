...
#
# Changes in v2.1:
#   - generate_baseline()         — zero-shot Control B response, called at session start
#   - assemble_evaluation_payload() — packages evaluation fields into blackboard at session end
#   - create_blackboard()         — now includes empty evaluation_payload field in schema
#
# Changes in v2.0:
...
def create_blackboard(user_prompt, system_version="2.1",
    ...
    v2.1 addition: evaluation_payload field at root level.
    Populated by assemble_evaluation_payload() at session end.
    """
    return {
        "session_metadata": {
        ...
        "reasoning_trace": [],
        "evaluation_payload": {}   # Populated at session end by assemble_evaluation_payload()
    }
...
def generate_baseline(user_prompt, model=DEFAULT_MODEL):
    ...
    Called ONCE at session start, immediately after initial_prompt is
    captured and before any agents or discovery stages run.

    This is Control B: raw input → single model call, no system prompt,
    no studio context, no discovery, no multi-agent structure. Used by
    the evaluator to measure the full end-to-end value of the system.

    The model should match SESSION_MODEL — same capability level as the
    Creative Team — so architecture is the isolated variable, not model
    capability.

    Returns: string — the raw baseline response text
    """
    response = client.messages.create(
        model=model,
        max_tokens=800,
...
def assemble_evaluation_payload(blackboard, baseline_response):
    """
    Assemble the evaluation_payload block and inject it into the blackboard.
    Called ONCE at session end, after final_synthesis is written and
    before save_session().

    Packages the three fields the evaluator needs for comparison:
      - problem_prompt:               the original raw user input
      - control_group_baseline:       the zero-shot response from session start
      - experimental_group_synthesis: the final Creative Prism synthesis

    This is a purely additive operation. All existing blackboard fields
    are untouched. The evaluation_payload field is initialized as an
    empty dict in create_blackboard() and populated here.
    """
